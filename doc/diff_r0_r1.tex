...
%DIF DEL northern_sill_energetics_r0.tex   Wed Feb 15 12:41:19 2023
%DIF ADD northern_sill_energetics.tex      Wed Feb 15 12:41:32 2023
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%DIF 58-64c58-64
%DIF < Jesse M. Cusack,\aff{a,b}
%DIF < Larry J. Pratt,\aff{c}
%DIF < James B. Girton,\aff{d}
%DIF < Glenn S. Carter,\aff{e}
%DIF < Jody M. Klymak,\aff{f}
%DIF < Shuwen Tan,\aff{g}
%DIF <  and Andreas M. Thurnherr\aff{g}
%DIF -------
Jesse M. Cusack,\aff{a,b,c} %DIF > 
Larry J. Pratt,\aff{d} %DIF > 
James B. Girton,\aff{e} %DIF > 
Glenn S. Carter,\aff{f} %DIF > 
Jody M. Klymak,\aff{g} %DIF > 
Shuwen Tan,\aff{h,i} %DIF > 
 and Andreas M. Thurnherr\aff{h} %DIF > 
%DIF -------
}
...
%DIF 69-73c69-75
%DIF < \aff{c}{Woods Hole Oceanographic Institution, Woods Hole, Massachusetts}\\
%DIF < \aff{d}{Applied Physics Laboratory, University of Washington, Seattle, Washington}\\
%DIF < \aff{e}{Department of Oceanography, University of Hawaii at M\={a}noa, Honolulu, Hawaii}\\
%DIF < \aff{f}{University of Victoria, Victoria, British Columbia, Canada}\\
%DIF < \aff{g}{Lamont-Doherty Earth Observatory, Columbia University, Palisades, New York}\\
%DIF -------
\aff{c}{College of Oceanic and Atmospheric Sciences, Oregon State University, Corvallis, Oregon} %DIF > 
\aff{d}{Woods Hole Oceanographic Institution, Woods Hole, Massachusetts}\\ %DIF > 
\aff{e}{Applied Physics Laboratory, University of Washington, Seattle, Washington}\\ %DIF > 
\aff{f}{Department of Oceanography, University of Hawaii at M\={a}noa, Honolulu, Hawaii}\\ %DIF > 
\aff{g}{University of Victoria, Victoria, British Columbia, Canada}\\ %DIF > 
\aff{h}{Lamont-Doherty Earth Observatory, Columbia University, Palisades, New York}\\ %DIF > 
\aff{i}{University of California Irvine, Irvine, California}\\ %DIF > 
%DIF -------
}
...
In this study we estimate the energy and momentum budget of flow across a major sill in the Samoan Passage using high \DIFaddbegin \DIFadd{horizontal }\DIFaddend resolution, towed, observations.
...
\DIFdelbegin \DIFdel{Both }\DIFdelend \DIFaddbegin \DIFadd{Flow steadiness is discussed as a precursor to the following analysis in section~\ref{sec:Results}\ref{ssec:FlowSteadiness}.
Both energy }\DIFaddend frameworks are applied to the observed and modeled overflow \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{to obtain }\DIFaddend energy budgets in section~\ref{sec:Results}\ref{ssec:EnergyBudgetResults}.
...
% Towyo Observations
\begin{figure*}
\DIFdelbeginFL %DIFDELCMD < \centerline{\includegraphics[width=39pc]{towyo_overview.pdf}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \centerline{\includegraphics[width=33pc]{towyo_overview.pdf}}
\DIFaddendFL \caption{
...
({\bf c}) \DIFaddbeginFL \DIFaddFL{Vertical velocity $w$ (color) and $\sigma_4$ isopycnals at $10^{-2}$\,kg\,m$^{-3}$ intervals.
(}{\bf \DIFaddFL{d}}\DIFaddFL{) }\DIFaddendFL Square of vertical shear $(\partial v/\partial z)^2$ (color) and \DIFdelbeginFL \DIFdelFL{isopycnal }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{isopycnals }\DIFaddendFL from panel \DIFdelbeginFL \DIFdelFL{b}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{c}\DIFaddendFL .
({\bf \DIFdelbeginFL \DIFdelFL{d}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{e}\DIFaddendFL }) Turbulent dissipation $\varepsilon$ from Thorpe-scale estimates.
({\bf \DIFdelbeginFL \DIFdelFL{e}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{f}\DIFaddendFL }) Barotropic tide prediction \citep[TPXO,][]{egberterofeeva02} for times and locations along the section.
...
(b) \DIFdelbeginFL \DIFdelFL{Northward }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Horizontal }\DIFaddendFL velocity.
(c) Vertical velocity.
...
%DIFDELCMD < \Delta F &= Q_u B_u -Q_d B_d = Q(B_u - B_d) \\
%DIFDELCMD <     &= v_u d_u (\frac{v_u^2}{2} + g^\prime \delta_u - \frac{v_d^2}{2} -g^\prime \delta_d)\ ,
%DIFDELCMD < \end{split}%%%
...
b) Baroclinic bottom pressure in observations and model offset by constant factors \DIFaddbeginFL \DIFaddFL{(model:~60\,N\,m$^{-2}$, towyo~2012:~-130\,N\,m$^{-2}$, towyo~2014:~-150\,N\,m$^{-2}$) }\DIFaddendFL for visualization purposes.
\DIFaddbeginFL \DIFaddFL{This relatively large offset in absolute baroclinic bottom pressure between model and observations is due to model initialization with the observed density structure and partial draining of the initial stratification during model spinup as discussed in section~\ref{sec:ExperimentalDetails}\ref{sec:Model}.
}\DIFaddendFL Faint black lines show bottom pressure for each model time step of the analysis period.
...
\DIFaddbegin \DIFadd{We thank two anonymous reviewers for their comments that helped improve the paper.
}\DIFaddend 
...
These terms are \DIFdelbegin \DIFdel{small and therefore neglected in the budget .
Estimates of diffusive fluxes }\DIFdelend \DIFaddbegin \DIFadd{inherently small as the bulk of the mixing is accomplished through the KL10 mixing parameterization (see section~\ref{sec:ExperimentalDetails}\ref{sec:Model}).
Even when calculated with the much larger turbulent diffusivities from the KL10 parameterization, which are calculated as $K_z = \Gamma \varepsilon N^{-2}$ based on a flux coefficient $\Gamma=0.2$ \mbox{%DIFAUXCMD
\citep{klymaklegg10}}\hskip0pt%DIFAUXCMD
, the diffusive fluxes of baroclinic potential and kinetic energy are only $\mathcal{O}(1)$\,W\,m$^{-1}$ when integrated over the budget boundaries.
They are thus orders of magnitude smaller than other budget terms and therefore neglected.
Diffusive fluxes of baroclinic potential and kinetic energy }\DIFaddend in the observations \DIFdelbegin \DIFdel{are also small and neglected}\DIFdelend \DIFaddbegin \DIFadd{can be estimated in a similar way and are of the same order of magnitude as in the model as the input parameters for the calculation are of similar size.
We note that using a larger flux coefficient, as may be necessary for near-bottom turbulence \mbox{%DIFAUXCMD
\citep[e.g.][]{ijichietal20,spingysetal21}}\hskip0pt%DIFAUXCMD
, does not change the orders-of-magnitude difference to other terms in the energy budget}\DIFaddend .
